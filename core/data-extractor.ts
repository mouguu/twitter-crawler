...
import type { ProfileInfo, RawTweetData, Tweet } from '../types/tweet-definitions';
        ...
        .map((article) => {
          try {
            ...
            const likeCountSpan = likeButton?.querySelector(
              `${SELECTORS.SHARE} span > span`,
            ) as HTMLElement | null;
            const retweetCountSpan = retweetButton?.querySelector(
              `${SELECTORS.SHARE} span > span`,
            ) as HTMLElement | null;
            const replyCountSpan = replyButton?.querySelector(
              `${SELECTORS.SHARE} span > span`,
            ) as HTMLElement | null;
            ...
            if (replyContext?.innerText.includes('Replying to')) {
              isReply = true;
            }
 ...
 * 检查页面上推文数量是否增长
 */
export async function waitForNewTweets(
  page: Page,
  previousCount: number,
  timeout: number = SCROLL_CONFIG.waitForNewTweetsTimeout,
): Promise<boolean> {
  try {
