...
// Task tracking removed - simplified to Job-level only
...
export async function runTimelineDateChunks(
  ...
  const ranges = DateUtils.generateDateRanges(
  ...
  if (isParallel) {
    engine.eventBus.emitLog(
      `Generated ${ranges.length} date chunks for historical search (Newest -> Oldest). Parallel processing enabled: ${parallelChunks} chunks simultaneously.`,
    );
  } else {
    engine.eventBus.emitLog(
      `Generated ${ranges.length} date chunks for historical search (Newest -> Oldest).`,
    );
  }
  ...
  if (isParallel) {
    ...
    const processChunksInParallel = async () => {
      const _chunkTasks: Array<{
        index: number;
        promise: Promise<{ success: boolean; tweets: Tweet[]; error?: string }>;
      }> = [];
      let currentIndex = 0;
      ...
      while (currentIndex < ranges.length) {
        ...
        engine.eventBus.emitLog(
          `[Parallel] Starting batch: ${batch.length} chunks (${batch.map((i) => i + 1).join(', ')})`,
        );
        ...
        const batchPromises = batch.map(async (i) => {
          ...
          engine.eventBus.emitLog(
            `[Parallel] Processing chunk ${i + 1}/${ranges.length}: ${range.start} to ${range.end}`,
          );

          // Task tracking removed - date chunking continues without DB overhead
          ...
          if (currentGlobalCount >= globalLimit) {
            engine.eventBus.emitLog(
              `[Parallel] Global limit ${globalLimit} already reached, skipping chunk ${i + 1}`,
              'info',
            );
          ...
          wrappedEventBus.emitProgress = (data: any) => {
            ...
            if (globalTotal >= globalLimit) {
              ...
              engine.eventBus.emitLog(
                `✅ Global limit of ${globalLimit} reached. This chunk should stop.`,
                'info',
              );
            }
        ...
        for (const { index, result } of batchResults) {
          ...
          if (result.success) {
            if (result.tweets && result.tweets.length > 0) {
              ...
              engine.eventBus.emitLog(
                `✅ [Parallel] Chunk ${index + 1}/${ranges.length} complete: ${newTweets.length} tweets collected | Global total: ${cappedTotal}/${globalLimit}`,
              );

              // 如果达到全局限制，记录日志
              if (totalCollected >= globalLimit) {
                engine.eventBus.emitLog(
                  `✅ Global limit of ${globalLimit} reached after chunk ${index + 1}. Stopping batch.`,
                  'info',
                );
              }
            } else {
              engine.eventBus.emitLog(
                `✅ [Parallel] Chunk ${index + 1}/${ranges.length} complete: No tweets found (empty).`,
                'info',
              );
            }
          } else {
    ...
    for (let i = 0; i < ranges.length; i++) {
      ...
      engine.eventBus.emitLog(
        `Processing chunk ${i + 1}/${ranges.length}: ${range.start} to ${range.end}`,
      );
      ...
      if (result.success) {
        if (result.tweets && result.tweets.length > 0) {
          ...
          engine.eventBus.emitLog(
            `✅ Chunk ${i + 1}/${ranges.length} complete: ${newTweets.length} tweets collected | Global total: ${totalCollected}/${globalLimit}`,
          );
        } else {
          engine.eventBus.emitLog(
            `✅ Chunk ${i + 1}/${ranges.length} complete: No tweets found (empty).`,
            'info',
          );
        }
      } else {
  ...
  if (failedChunks.length > 0 && engine.isRotationEnabled()) {
    ...
    const stillFailed = failedChunks.filter(
      (chunk) => chunk.retryCount <= CHUNK_RETRY_CONFIG.maxGlobalRetries,
    );
