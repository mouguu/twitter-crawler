...
export class BrowserManager {
  ...
  async newPage(options: BrowserLaunchOptions = {}): Promise<Page> {
    ...
    if (options.proxy) {
      ...
      console.log(
        `[BrowserManager] Proxy authentication injected for ${options.proxy.host}:${options.proxy.port}`,
      );
    }
  ...
  async setupRequestInterception(blockedTypes: string[] | null = null): Promise<void> {
    ...
    try {
      ...
      console.warn(
        '[BrowserManager] Failed to enable CDP blocking, falling back to standard interception',
        e,
      );
    }
